...
def delete_question():
    mysql_cursor.execute(f"DELETE FROM frage_table WHERE frage_id=' ' OR frage IS NULL;") #delete accidental empty values


def search_event(): # button event handling function
    print("SEARCH")
    try:
        search_event_sql = mysql_cursor.execute(f"SELECT * FROM frage_table, antwort_table, nutzer_table WHERE{searchbox.get()}")
    except:
...
def add_question():
    ...
    try:
        ...
        tkinter.messagebox.showerror("Datenbankfehler", "Ihre Frage konnte nicht zur Datenbank hinzugefügt werden!")

def add_answer():
    print("FRAGE BEANTWORTEN")
    add_answer_sql = f"""
    INSERT INTO antwort_table (antwort,antwort_datum,{nutzer_id})
...
ask_question_frame = customtkinter.CTkFrame(master=search_util_container,width=current_wh_list[0]*3, height=50, corner_radius=15) #frame which stores entry aswell as button(s)
...
ask_question_button = customtkinter.CTkButton(master=ask_question_frame,width=current_wh_list[0]*3,height=35, text="Frage stellen", command=add_question)
ask_question_button.pack(side="bottom", pady=10, padx=10, fill="both")

utility_frame = customtkinter.CTkFrame(master=search_util_container, width=current_wh_list[0]*3, height=50, corner_radius=15) #frame which stores entry aswell as button(s)
utility_frame.pack(side="bottom",pady=5, padx=5, fill="both")

edit_button = customtkinter.CTkButton(master=utility_frame, width=50,height=35,text="Bearbeiten", command=edit_data)
edit_button.pack(side="bottom",pady=10,padx=10,fill="both")
...
tree.column("frage",minwidth=400)
tree.column("frage_datum",minwidth=45)
tree.column("beantwortet",minwidth=85)
tree.column("antwort_id",minwidth=75)
tree.column("tags",minwidth=45)
...
main_content_frame_right_bottom = customtkinter.CTkFrame(master=main_content_frame_container,height=200, width=500, corner_radius=15)
...
tree2_columns = ("antwort_id","antwort_datum","antwort","frage_id")
tree2 = tk.ttk.Treeview(main_content_frame_right_top,style="mystyle.Treeview", height=250, columns=tree2_columns, show='headings')
#print(current_wh_list[1])
tree2.column("antwort_id",minwidth=55)
tree2.column("antwort_datum",minwidth=55)
tree2.column("antwort",minwidth=250)
...
tree2.heading("frage_id", text="Frage ID",anchor=tk.W)
...
def query_frage_table():
    ...
    for record in frage_table_records:
        ...
        ##edit_button = customtkinter.CTkButton(tree, text="Edit " + id_reference, width=25,height=15)
...
def query_antwort_table():
    ...
    for record in antwort_table_records:
        tree.insert(parent="",index="end", iid=record[0], values=(record[0],record[1],record[2],record[3],record[4],record[5],record[6])) # tags="evenrow"
query_antwort_table()

def update_treeview_frage_table():
    mysql_cursor.execute("SELECT * FROM frage_table ORDER BY frage_id;") 
    frage_table_records = mysql_cursor.fetchall()
    ...
    try:
        ...
        print("Couldn't update treeview!")

refresh_button = customtkinter.CTkButton(master=searchbar_frame_options, width=60,height=30,text="Aktualisieren", command=update_treeview_frage_table) #press this button to refresh the view
...
##def on_doublew_click(self,event):
##    item = self.tree.selection()
##    for i in item:
##        print("you clicked on", self.tree.item(i, "values)[0]"))

# NOTE: trying to create my own row/column data display has mostly resulted in a failure, therefore i will use treeview for now
### 0
##tabular_header0_frame = customtkinter.CTkFrame(master=main_content_frame_left, corner_radius=15) 
##tabular_header0_frame.grid(row=0, column=0,pady=5, padx=5)
##tabular_header0 = customtkinter.CTkLabel(master=tabular_header0_frame, height=20,corner_radius=15,text="Frage ID" )
##tabular_header0.grid(row=0, column=0)
##
### 1
##tabular_header1_frame = customtkinter.CTkFrame(master=main_content_frame_left, corner_radius=15) 
##tabular_header1_frame.grid(row=0, column=1,pady=5, padx=5)
##tabular_header1 = customtkinter.CTkLabel(master=tabular_header1_frame, height=20,corner_radius=15,text="Frage" )
##tabular_header1.grid(row=0, column=1)
##
### 2
##tabular_header2_frame = customtkinter.CTkFrame(master=main_content_frame_left, corner_radius=15) 
##tabular_header2_frame.grid(row=0, column=2,pady=5, padx=5)
##tabular_header2 = customtkinter.CTkLabel(master=tabular_header2_frame, height=20,corner_radius=15,text="Datum" )
##tabular_header2.grid(row=0, column=2)
##
### 3
##tabular_header3_frame = customtkinter.CTkFrame(master=main_content_frame_left, corner_radius=15) 
##tabular_header3_frame.grid(row=0, column=3,pady=5, padx=5)
##tabular_header3 = customtkinter.CTkLabel(master=tabular_header3_frame, height=20,corner_radius=15,text="Beantwortet?" )
##tabular_header3.grid(row=0, column=3)
##
### 4
##tabular_header4_frame = customtkinter.CTkFrame(master=main_content_frame_left, corner_radius=15) 
##tabular_header4_frame.grid(row=0, column=4,pady=5, padx=5)
##tabular_header4 = customtkinter.CTkLabel(master=tabular_header4_frame, height=20,corner_radius=15,text="Antwort ID" )
##tabular_header4.grid(row=0, column=4)
##
### 5
##tabular_header5_frame = customtkinter.CTkFrame(master=main_content_frame_left, corner_radius=15) 
##tabular_header5_frame.grid(row=0, column=5,pady=5, padx=5)
##tabular_header5 = customtkinter.CTkLabel(master=tabular_header5_frame, height=20,corner_radius=15,text="Tags" )
##tabular_header5.grid(row=0, column=5)
##
### 6
##tabular_header6_frame = customtkinter.CTkFrame(master=main_content_frame_left, corner_radius=15) 
##tabular_header6_frame.grid(row=0, column=6,pady=5, padx=5)
##tabular_header6 = customtkinter.CTkLabel(master=tabular_header6_frame, height=20,corner_radius=15,text="Nutzer ID" )
##tabular_header6.grid(row=0, column=6)

# main content frame "right_top"

# main content frame _right_bottom
...
def tasks(): # consider this like a "secondary" loop to run any functions in
    ...
    ##print(current_focus_tree1,current_focus_tree2)
